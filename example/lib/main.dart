...
class _ShowcasePageState extends State<ShowcasePage> {
        ...
        children: [
          ...
          Row(
            children: [
              ...
              if (code != null)
                TextButton.icon(
                  onPressed: () => setState(() {
                    expanded ? _codeVisible.remove(text) : _codeVisible.add(text);
                  }),
          ...
          if (code != null)
            AnimatedSize(
              ...
              child: expanded ? _codePanel(code) : const SizedBox(width: double.infinity),
            ),
        ],
      ),
    );
  ...
  TextSpan _highlightPlinthCode(String code) {
    ...
    for (final match in _plinthIdentifier.allMatches(code)) {
      if (match.start > lastEnd) {
        spans.add(TextSpan(text: code.substring(lastEnd, match.start), style: _codeBaseStyle));
      }
  ...
  Widget _plinthMark({double scale = 1}) {
    return Column(
      ...
      children: [
        Container(width: 22 * scale, height: 5 * scale, color: const Color(0xFF228BE6)),
        const SizedBox(height: 2),
        Container(width: 30 * scale, height: 5 * scale, color: const Color(0xFF228BE6)),
        const SizedBox(height: 2),
        Container(width: 38 * scale, height: 5 * scale, color: const Color(0xFF228BE6)),
      ],
    );
  }
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const PlinthBadge('Web + Desktop', color: 'green', variant: PlinthVariant.outline),
            ],
          ),
        ],
      ),
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                const Text('Plinth UI', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 18)),
              ],
            ),
          ),
