...
with a real API, which an app installs and calls. Eleven are there so
far — authentication, the error states, banners and the FAQ.
...
**`example/lib/src/showcase/`** is where the other 93 still are, as
