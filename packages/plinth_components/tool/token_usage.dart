// Scans the widget sources for token reads and renders docs/TOKENS.md.
//
// A library rather than a script body so that
// `token_usage_fresh_test.dart` can call it **in process**. The first
// version had the test spawn `flutter test tool/…`, which deadlocked
// CI: melos runs packages in parallel, each holding the Flutter SDK's
// startup lock, and a nested `flutter` invocation waits for a lock the
// process around it is already holding.
//
// Re-implementing the scan inside the test was the alternative and is
// worse: a freshness check that duplicates the generator drifts from
// it, which is the failure it exists to prevent, one level up.
...
const tokensDocPath = '../../docs/TOKENS.md';

/// Builds the document. Pure: reads sources, returns text, writes
/// nothing — which is what lets the freshness test compare without
/// touching the working tree.
String buildTokenUsageDoc() {
  final dir = Directory('lib/src/widgets');
  if (!dir.existsSync()) {
    throw StateError('Run this from packages/plinth_components.');
  }
  ...
  return _render(readers, dynamics);
}
