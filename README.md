...
- Extend the composed-blocks showcase — 63 examples against Mantine
  UI's ~123. Every subcategory now has something in it except
  Carousels (a deliberate scope call), so what remains is depth. Stats
  is now the widest gap at 9 against 3. Nothing is blocked on a
  missing component. See **[docs/SHOWCASE.md](docs/SHOWCASE.md)**.
