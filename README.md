...
for all 97 components, and the theme-token table there explains which
...
- **97 components** across Primitives, Forms, Feedback, Data Display,
...
- **Widgetbook gallery** (`widgetbook/`) — manual (non-codegen)
  ...
  with a knob-driven **Playground** for 92 of the 97 components
  alongside the static variant grids (the two answer different
  questions: a playground explores combinations, a grid compares
  options side by side). A smoke test builds all 205 use cases in CI,
...
- **Tests** — pure-logic tests for `PlinthTheme`'s shade generator and
  ...
  sizes), and widget behavior tests across 41 test files. **Every
