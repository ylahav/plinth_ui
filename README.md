...
for all 92 components, and the theme-token table there explains which
...
- **92 components** across Primitives, Forms, Feedback, Data Display,
...
- **Widgetbook gallery** (`widgetbook/`) — manual (non-codegen)
  ...
  with a knob-driven **Playground** for 87 of the 92 components
  alongside the static variant grids (the two answer different
  questions: a playground explores combinations, a grid compares
  options side by side). A smoke test builds all 197 use cases in CI,
...
- **Tests** — pure-logic tests for `PlinthTheme`'s shade generator and
  ...
  sizes), and widget behavior tests across 40 test files. **Every
