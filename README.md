...
- Extend the composed-blocks showcase — 66 examples against Mantine
  UI's ~123. Every subcategory now has something in it except
  Carousels (a deliberate scope call), so what remains is depth in the
  ones nobody has needed yet. Nothing is blocked on a missing
  component. See **[docs/SHOWCASE.md](docs/SHOWCASE.md)**.
